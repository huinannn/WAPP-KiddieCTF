        ...
        protected void btnPost_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(title) || (string.IsNullOrEmpty(description) && !fileImage.HasFile))
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert",
                    "Swal.fire({ icon: 'warning', title: 'Incomplete', text: 'Please add a title and either a description or an image.' });", true);
                return;
            }
            ...
            if (fileImage.HasFile)
            {
                ...
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "alert",
                        "Swal.fire({ icon: 'error', title: 'Invalid File', text: 'Only JPG, JPEG, PNG, and GIF images are allowed.' });", true);
                    return;
                }
                ...
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "alert",
                        "Swal.fire({ icon: 'error', title: 'Invalid File', text: 'Please upload an image file only.' });", true);
                    return;
                }
            ...
            // ✅ Use SweetAlert2 for success and redirect after confirmation
            ClientScript.RegisterStartupScript(this.GetType(), "success",
                "Swal.fire({ icon: 'success', title: 'Posted!', text: 'Your discussion was posted successfully.' })" +
                ".then(() => { window.location='Discussions.aspx'; });", true);
        }
    }
}
